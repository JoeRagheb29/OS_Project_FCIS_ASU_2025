...
//    TODO: [PROJECT'25.IM#2] USER HEAP - #2 allocate_user_mem
	...
	uint32* ptr_page_directory = e->env_page_directory;

	uint32 rounded_size = ROUNDDOWN(size, PAGE_SIZE);
	uint32 Range = virtual_address + rounded_size;


	for(uint32 v = virtual_address; v < Range ; v+=PAGE_SIZE) {
		...
		uint32 checkPage = get_page_table(ptr_page_directory, v, &ptr_Page_Table);
		if(ptr_Page_Table == NULL) {
			ptr_Page_Table = create_page_table(ptr_page_directory , v);
		}

//		if(ptr_Page_Table == NULL) {
//			cprintf("ptr page table = null ya ragheb");
//			return ;
//		}

//		map_frame(ptr_page_directory, frame_info, v, PERM_USER | PERM_WRITEABLE | PERM_UHPAGE);

		// assign in its permissions the three flags
		pt_set_page_permissions(ptr_page_directory ,v, PERM_UHPAGE | PERM_AVAILABLE, 0);
...
//		return;
	...
	for (uint32 v = virtual_address; v < Range; v += PAGE_SIZE) {
		...
		pt_set_page_permissions(e->env_page_directory, v, 0, PERM_UHPAGE | PERM_AVAILABLE);

		pf_remove_env_page(e, v);
	}
